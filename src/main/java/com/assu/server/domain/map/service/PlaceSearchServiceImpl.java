...
public class PlaceSearchServiceImpl implements PlaceSearchService {
    ...
    @Override
    public List<PlaceSuggestionDTO> unifiedSearch(String query, Integer size) {
        ...
        List<PlaceSuggestionDTO> kwList = convertKeyword(kw);

        // 3) 좌표가 있으면 카테고리 근접 검색 보강 (음식점/카페 등)
        List<PlaceSuggestionDTO> nearby = Collections.emptyList();
        if (x != null && y != null) {
            List<String> cats = List.of("FD6", "CE7"); // 음식점/카페 (필요시 카테고리 추가)
            List<PlaceSuggestionDTO> merged = new ArrayList<>();
            for (String c : cats) {
                var r = kakaoLocalClient.searchByCategory(c, x, y, NEARBY_DEFAULT_RADIUS, 1, kSize);
                merged.addAll(convertKeyword(r));
            }
            // 거리 오름차순
            merged.sort(Comparator.comparing(dto -> Optional.ofNullable(dto.distance()).orElse(Integer.MAX_VALUE)));
            nearby = merged;
        }

        // 4) 결과 합치기 (키워드 우선 → 근접 결과 추가, id로 dedupe)
        Map<String, PlaceSuggestionDTO> dedupe = new LinkedHashMap<>();
        Stream.concat(kwList.stream(), nearby.stream())
                .forEach(dto -> dedupe.putIfAbsent(dto.placeId(), dto));

        // 최종 상위 size 개 제한
        return dedupe.values().stream().limit(kSize).toList();
    }

    private List<PlaceSuggestionDTO> convertKeyword(KakaoLocalClient.KakaoKeywordResp resp) {
        if (resp == null || resp.getDocuments() == null) return List.of();
        List<PlaceSuggestionDTO> out = new ArrayList<>();
        for (var d : resp.getDocuments()) {
            Double lng = safeParse(d.getX());
            Double lat = safeParse(d.getY());
            Integer dist = safeParseInt(d.getDistance()); // null 가능
            out.add(new PlaceSuggestionDTO(
                    d.getId(),
                    d.getPlace_name(),
                    d.getCategory_group_name() != null ? d.getCategory_group_name() : d.getCategory_name(),
                    d.getAddress_name(),
